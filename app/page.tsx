...
function DashboardInner({ name, user }: { name: string; user: User }) {
  ...
  async function handleTogglePaid(billId: string, nextPaid: boolean, amount?: number) {
    ...
    // If the user changed the amount, update the bill's default for next time
    const bill = bills.find((b) => b.id === billId);
    if (bill && amount !== undefined && amount !== bill.amount) {
      await supabase.from("bills").update({ amount }).eq("id", billId);
    }

    if (bill) {
      const action = nextPaid ? "marked_paid" : "marked_unpaid";
      const verb = nextPaid ? "marked" : "un-marked";
      const amountNote = amount && amount !== bill.amount ? ` ($${amount.toFixed(2)})` : "";
      await supabase.from("notifications").insert({
        actor_name: name,
        actor_email: user.email ?? "",
        action,
        subject: bill.name,
        message: `${name} ${verb} ${bill.name} as ${nextPaid ? `paid ✓${amountNote}` : "unpaid"}`,
        read_by: [user.email ?? ""],
      });
    }
